...
void Synth::setWaveType(int type, int index)
{
    if (index >= AMOUNT_OF_OPERATORS)
    {
        return;
    }
    m_operators[index]->setWaveType(type);
}

void Synth::setVolume(int volume)
{
    for (int i = 0; i < AMOUNT_OF_OPERATORS; i++)
    {
        m_operators[i]->setVolume(volume);
    }
}
